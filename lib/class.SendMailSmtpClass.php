...
class SendMailSmtpClass {
    ...
    */
	...
	private function getContentMail($subject, $message, $smtp_from, $mailTo){	
		...
		$headers .= "From: =?" . $this->smtp_charset . "?B?" . base64_encode($smtp_from[0]) . "?= <" . $smtp_from[1] . ">\r\n"; // от кого письмо
		$headers.= "To: ".$mailTo."\r\n"; // кому
        $contentMail .= $headers . "\r\n";
		
		//$message = $this->getClearHtml($message);
		if($this->addFile){
			// если есть файлы
	...
	// Убираем лишние пробелы в html
	private function getClearHtml($html) {
	    $html = preg_replace('~\s+~mui', ' ', $html);
	    $replace_arr = array(
	        '> <' => '><',
	        'p> ' => 'p>',
	        ' </p' => '</p',
	    );
	    return trim(str_replace(array_keys($replace_arr), $replace_arr, $html));
	}
